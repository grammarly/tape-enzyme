...
Check if the current `wrapper` has an `element` that matches given selector anywhere in its render tree.
...
Check if the current `wrapper` doesn't have an `element` that matches given selector anywhere in its render tree.
...
Check if the current `wrapper` doesn't have a `node` anywhere in it's render tree that looks like the one passed in.
...
Check whether or not the current `wrapper` has `nodes` anywhere in its render tree that look like the nodes passed in.
...
Check if the current node doesn't have a `className` prop including the passed in class name.
...
Check if the current `node` matches a provided `selector`.
